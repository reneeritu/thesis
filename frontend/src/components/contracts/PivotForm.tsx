...
export function PivotForm({ projectId, onDone }: Props) {
  ...
  return (
    <div className="border border-black bg-white p-4 space-y-3">
      <h3 className="text-small font-mono uppercase tracking-[0.18em]">Record Pivot</h3>
      <p className="text-small text-grey-400">A pivot records a change in direction. It does not stop the project.</p>
      <form onSubmit={onSubmit} className="space-y-3 text-small">
        <div>
          <label className="block font-mono uppercase tracking-[0.18em] text-grey-400 mb-1">What changed</label>
          <textarea
            value={reason}
