    ...
    <AppShell title="Discover">
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {query && (
            <button
              type="button"
              onClick={() => setQuery('')}
              className="border border-white/25 bg-zinc-900/55 px-2 py-2 font-mono text-[11px] uppercase tracking-[0.14em] hover:bg-white/10"
            >
              Clear
            </button>
          )}
        ...
        {current.loading && current.items.length === 0 && (
          <p className="font-mono text-[11px] text-white">Loading…</p>
        )}
                  ...
                  <li key={s._id} className="px-4 py-3 space-y-1">
                    ...
                    {!s.isMember && s.projectAccess === 'open' && (
                      <Link
                        ...
                        className="inline-block border border-white/25 bg-zinc-900/55 px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.14em] hover:bg-black hover:text-yellow-400 transition"
                      >
                        Join
                      </Link>
                    )}
                  ...
                  <li key={n.alias} className="px-4 py-3 flex items-start justify-between gap-2">
                    <div>
                      ...
                      {(n.interests.length > 0 || n.keywords.length > 0) && (
                        <p className="font-mono text-[10px] text-white mt-0.5 break-words">
        ...
        {hasMore && (
          <div className="flex justify-center">
            <button
              ...
              onClick={onLoadMore}
              className="border border-white/25 bg-zinc-900/55 px-4 py-2 font-mono text-[11px] uppercase tracking-[0.18em] hover:bg-black hover:text-yellow-400 transition disabled:opacity-50"
            >
