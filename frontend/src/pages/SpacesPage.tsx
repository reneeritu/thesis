...
function SpaceCard({
  ...
  return (
    <Link
      ...
      className={`group flex min-h-[280px] flex-col overflow-hidden rounded-sm border border-white/12 bg-black/35 transition hover:border-white/22 ${
    ...
    >
      ...
      <div className="flex min-h-0 flex-[2] flex-col gap-1.5 bg-black/82 px-3 py-3">
        <h2 className="m-0 font-mono text-[15px] font-medium uppercase tracking-[0.12em] leading-snug text-white/92 line-clamp-2">
          {space.name}
        </h2>
        ...
        <p className="m-0 font-mono text-[10px] text-white/42">
          {typeof space.memberCount === 'number' ? `${space.memberCount} member${space.memberCount === 1 ? '' : 's'}` : '—'}
        </p>

        {desc ? (
          <p className="m-0 line-clamp-2 font-mono text-[11px] italic leading-relaxed text-white/38">{desc}</p>
        ) : null}

        <p className="mt-auto pt-1 font-mono text-[10px] uppercase tracking-[0.2em] text-white/38 transition group-hover:text-white/55">
          Open →
        </p>
      </div>
    </Link>
    ...
    <AppShell title="Spaces">
      <div className="flex w-full min-h-0 flex-1 flex-col gap-0 pb-4">
        ...
        <header className="flex w-full flex-wrap items-start justify-between gap-4">
          ...
          <nav className="flex flex-wrap items-center justify-end gap-2 font-mono text-[11px] uppercase tracking-[0.16em]">
            <Link
              to="/spaces/new"
        ...
        {spaces.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-5 py-16 text-center">
            <p className="m-0 max-w-md font-mono text-small leading-relaxed text-white/65">
              You&apos;re not part of any spaces yet.
            </p>
            <div className="flex flex-wrap items-center justify-center gap-3 font-mono text-[11px] uppercase tracking-[0.16em]">
              <Link
                to="/spaces/join"
                className="border border-white/22 bg-black/30 px-4 py-2 text-white/88 transition hover:border-white/45 hover:text-white"
              >
                + Join
              </Link>
              <Link
